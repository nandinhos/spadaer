...
<div
    x-data="{
        get m() {
            return Alpine.store('modals') || {
                showDocumentDetails: false, loadingDocument: false, selectedDocument: {},
...
>
    ...
    <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:items-center sm:block sm:p-0">
        ...
        <div 
            ...
            x-on:click="m.closeDocumentDetails()" 
            aria-hidden="true"
        ></div>
        ...
        >
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-400"
                               x-text="m.selectedDocument?.document_number || 'Carregando...'">
                            </p>
                        </div>
                    </div>
                    <button
            ...
            {{-- Corpo --}}
            <div class="px-6 py-6">
                <div x-show="m.loadingDocument" class="flex flex-col items-center justify-center py-12">
                    <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div>
                    <p class="mt-4 text-sm text-gray-500">Recuperando informações...</p>
                </div>

                <div x-show="!m.loadingDocument && m.selectedDocument?.id" x-transition>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {{-- Seção de Identificação --}}
                        <div class="space-y-4">
                            <div>
                                <label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Título</label>
                                <p class="text-sm font-medium text-gray-900 dark:text-gray-100 mt-1" x-text="m.selectedDocument?.title || '---'"></p>
                            </div>
                            <div>
                                <label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Projeto</label>
                                <p class="text-sm font-medium text-gray-900 dark:text-gray-100 mt-1" x-text="m.selectedDocument?.project?.name || 'Não associado'"></p>
                            </div>
                            <div>
                                <label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Data do Documento</label>
                                <p class="text-sm font-medium text-gray-900 dark:text-gray-100 mt-1" x-text="m.selectedDocument?.document_date || '---'"></p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-900 dark:text-gray-100 mt-1">
                                    Caixa <span x-text="m.selectedDocument?.box?.number || '---'"></span> / Item <span x-text="m.selectedDocument?.item_number || '---'"></span>
                                </p>
                            </div>
                            <div>
                                ...
                                <div class="mt-1">
                                    <span
                                        ...
                                        :class="{
                                            'bg-red-50 text-red-700 border-red-100 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800': m.selectedDocument?.confidentiality === 'Confidencial',
                                            'bg-yellow-50 text-yellow-700 border-yellow-100 dark:bg-yellow-900/20 dark:text-yellow-400 dark:border-yellow-800': m.selectedDocument?.confidentiality === 'Restrito',
                                            'bg-green-50 text-green-700 border-green-100 dark:bg-green-900/20 dark:text-green-400 dark:border-green-800': m.selectedDocument?.confidentiality === 'Público'
                                        }"
                                        x-text="m.selectedDocument?.confidentiality || 'Não definido'">
                                    </span>
                                </div>
                            </div>
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-900 dark:text-gray-100 mt-1">
                                    v<span x-text="m.selectedDocument?.version || '1.0'"></span> — 
                                    <span x-text="m.selectedDocument?.is_copy ? 'Cópia' : 'Original'"></span>
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="px-6 py-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row-reverse gap-3">
                <x-ui.button variant="secondary" x-on:click="m.closeDocumentDetails()" class="w-full sm:w-auto">
                    Fechar
                </x-ui.button>
                ...
                @can('documents.edit')
                    <x-ui.button 
                        variant="primary"
                        x-show="m.selectedDocument?.id"
                        x-on:click="window.location.href='/documents/' + m.selectedDocument?.id + '/edit'"
